...
$fs_text_domain = 'instantsearch-for-woocommerce';
 ...
 */
class WCISPlugin {
    const SERVER_URL = 'https://woo.instantsearchplus.com/';
    const DASHBOARD_URL = 'https://woo.instantsearchplus.com/';

    const ISP_SERVER_ENDPOINT = 'https://api.fastsimon.com/';
    ...
    const CRON_SEND_CATEGORIES_TIME_INTERVAL = 30;      // -> 30 seconds
...
//         add_action( 'publish_post', array( $this, 'on_product_update' ));
        ...
        add_action('wclsi_update_product', array($this, 'on_wclsi_update_product'));
        ...
        add_action('woocommerce_rest_insert_product_object', array($this, 'woocommerce_rest_insert_product'));
        add_action('woocommerce_rest_delete_product_object', array($this, 'woocommerce_rest_delete_product'));
        /*
        ...
        */
        ...
        if (get_option('wcis_enable_rewrite_cats')) {
            if ( !function_exists('wp_is_block_theme') || !wp_is_block_theme() ) {
                ...
                add_filter('template_include', array($this, 'rewrite_category_template'), 1000);
            }
        }
        ...
        //HPOS compatibility
        add_action( 'before_woocommerce_init', array( $this, 'set_hpos_compatibility'));
    }

    public function woocommerce_rest_insert_product($product){
    ...
    public function woocommerce_rest_delete_product($product){
        if ($product->post_type == 'product') {
            $product_id = method_exists($product, 'get_id') ? $product->get_id() : $product->id;
    ...
    /* main function for overriding category pages.
       change location when a category location is identified */
    public function wcis_rewrite_cat_template($located) {
        if (strpos($located, 'archive-product.php') !== false &&
                WCISCategory::is_category_page()) {
            $located = WCISServing::get_serving_page();
        }
        return $located;
    }

    /* override category page for a specific theme when a category template is reached */
    function rewrite_category_template( $template, $load = true ) {
        $theme = wp_get_theme();
        $themes_list = ['Best Shop', 'SmartShop'];

        $is_theme_in_list =  (in_array($theme->name, $themes_list) ||
                              in_array($theme->parent_theme, $themes_list) );

        if ($is_theme_in_list &&
                strpos($template, 'woocommerce.php') !== false &&
                WCISCategory::is_category_page()) {
            $template = WCISServing::get_serving_page();
        }

        return $template;
    }

    public static function isp_update_option($option_key, $option_value){
        update_option($option_key, $option_value, false);
        wp_cache_delete($option_key, 'options');
    }

    private static function isp_delete_option($option_key){
...
//             'post_excerpt'   => __('Search Results','WCISPlugin'), //[ <string> ] // For all your post excerpt needs.
    ...
    private function single_activate($is_retry = false) {
        ...
        //set svelte as default version for new users
        WCISAutocomplete::set_autocomplete_version();
        WCISServing::set_serp_or_category_version('serp');
        WCISServing::set_serp_or_category_version('category');
    ...
    private function get_product_from_post($post_id)
    {
        ...
        if (!$woocommerce_ver_below_2_1){
            $this->check_for_scheduled_sales($post_id);
        }
        ...
        if (taxonomy_exists('brand')){
            foreach (wp_get_post_terms($post_id, 'brand') as $brand){
        ...
        if (taxonomy_exists('pwb-brand')){
            foreach (wp_get_post_terms($post_id, 'pwb-brand') as $brand){
                $product_brands[] = $brand->name;
            }
        }
        ...
        if ($product->is_type( 'composite' )
            ...
            if (!$product->is_type('variable')) {
                if (class_exists('WDP_Frontend')) {
                    if (class_exists('WDP_Functions') && method_exists('WDP_Functions', 'get_discounted_product_price')) {
                        $sale_price = WDP_Functions::get_discounted_product_price($product, 1);
                    } elseif (method_exists($product, 'get_sale_price')) {
                    ...
                    if ($sale_price && $sale_price != '') {
                        if (floatval($sale_price) < floatval($price) || !$price) {
                            $price = $sale_price;
                        }
                    }
                } else {
        ...
        if (strlen(strip_tags($post_data->post_content)) > 2048) {
            $description = substr(strip_tags($post_data->post_content), 0, 2048);
            $last_space_loc = strrpos($description, ' ', -1);
            if ($last_space_loc) {
                $description = substr($description, 0, $last_space_loc);
            }
        } else {
        ...
        if (strlen(strip_tags($post_data->post_excerpt)) > 2048) {
            $short_description = substr(strip_tags($post_data->post_excerpt), 0, 2048);
            $last_space_loc = strrpos($short_description, ' ', -1);
            if ($last_space_loc) {
                $short_description = substr($short_description, 0, $last_space_loc);
            }
        } else {
            $short_description = strip_tags($post_data->post_excerpt);
        }
        ...
        try{
            ...
            if ( $is_variable && ! empty($variation_ids ) ){
                ...
                foreach ($variation_ids as $variation_id){
                    ...
                    try {
                        foreach ($variant_attributes as $var_attr_name => $var_attr_val) {
                            ...
                            if (
                                ...
                                $full_term_name = $var_attr_val;
                                if ($full_term) {
                                    $full_term_name =  $full_term->name;
                                }
                                if (array_key_exists($real_attr_name, self::$attributes_cache)) {
                                    self::$attributes_cache[$real_attr_name][$var_attr_val] = $full_term_name;
                                } else {
                                    self::$attributes_cache[$real_attr_name] = array($var_attr_val => $full_term_name);
                                }
                            }
                    ...
                    if ($woocommerce_ver_above_3_0) {
                        if (!$variants_stock) {
                            $variants_stock = $variation->is_in_stock();
                        }
                        $variants_qty += $variation->get_stock_quantity();
                    }
                    ...
                    if (!$woocommerce_ver_below_2_1){
                        $this->check_for_scheduled_sales($post_id, $variation->get_id());
                    }
                ...
                foreach ($variant_attributes_codes as $attr_code) {
                    ...
                    if (array_key_exists($attr_code, self::$attributes_cache)) {
                ...
                if ($woocommerce_ver_above_3_0) {
                    $send_product['is_in_stock'] = $variants_stock;
                }
            }
            ...
            if (!$woocommerce_ver_above_3_0) {
                ...
                $send_product['total_variable_stock'] = $variants_qty;
            }
                ...
                if ( isset( $is_variable ) && isset($variation_ids) && $is_variable && count($variation_ids) > 0) {
                    if (!class_exists('WDP_Frontend')) {
                        ...
                        if (class_exists('WDP_Functions')
                            && method_exists('WDP_Functions', 'get_discounted_product_price')
                            && WDP_Functions::get_discounted_product_price($product, 1)) {

                            $sale_prices = WDP_Functions::get_discounted_product_price($product, 1);
                            $send_product['price_min'] = $sale_prices[0];
                            $send_product['price'] = $sale_prices[0];
                            $max_price = $sale_prices[1];
                        } else {
                            if (floatval($price) > 0) {
                                $send_product['price_min'] = min(floatval($product->get_variation_price('min')), floatval($price));
                                if (floatval($send_product['price_min']) < floatval($send_product['price'])) {
                                    $send_product['price'] = $send_product['price_min'];
                                }
                            } else {
                                $send_product['price_min'] = $product->get_variation_price('min');
                    ...
                    if ($max_price && $max_price > 0)
                        $send_product['price_max'] = $max_price;
                    else
...
//         $category = get_term_by( 'id', $category_id, 'product_cat', 'ARRAY_A' );
        ...
        $thumbnail_id = function_exists( 'get_term_meta' ) ?
            get_term_meta( $product_category->term_id, 'thumbnail_id', true )
            : get_metadata( 'woocommerce_term', $product_category->term_id, 'thumbnail_id', true );
        ...
        $category_name = html_entity_decode($product_category->name);
    ...
    public function on_product_import($product){
        if ($product->post_type == 'product') {
            $product_id = method_exists($product, 'get_id') ? $product->get_id() : $product->id;
            self::on_product_save($product_id);
        }
    ...
    public function on_wclsi_update_product($ls_prod) {
        if ($ls_prod->wc_prod_id) {
            $this->on_product_save($ls_prod->wc_prod_id);
        }
        return $ls_prod;
    }
    ...
    public function on_product_update($post_id, $action, $skip_direct_update=false){
        ...
        if ($timestamp != false){	// event already scheduled
            if ($products_list && count($products_list) > 0){	// if there is at least one product in the list
    ...
    function insert_element_to_cron_list($element_id, $action, $type_of_element = self::ELEMENT_TYPE_PRODUCT){
        ...
        if ($elements_list){	// the list already has products
            ...
            foreach ($elements_list as $p){
                if (intval($element_id) == intval($p['element_id'])){
                    $p['action'] = $action;
                    $insert_new = false;
                }
                $new_list[] = $p;
            ...
            if ($insert_new) {
                ...
                array_push($new_list, $element_node);
            }
        ...
        if ($type_of_element == self::ELEMENT_TYPE_CATEGORY){
            self::isp_update_option('cron_category_list', $new_list);
        } else {
            self::isp_update_option('cron_product_list', $new_list);
        }
    }
    ...
    public function enqueue_scripts() {
        ...
        WCISAutocomplete::enqueue_script($this->products_per_page);

        if (isset($wp_query) &&
                is_search() &&
                !$this->fulltext_disabled &&
                !WCISServing::is_svelte_serp()) {
            $serp_url = 'https://acp-magento.appspot.com/js/wcis-results.js';
            wp_enqueue_script($this->plugin_slug . '-fulltext', $serp_url . '?' . self::get_full_text_script_params(), array('jquery'), self::VERSION);
    ...
    function add_async_defer_attributes($tag, $handle){
        $autocomplete_script_id = WCISAutocomplete::get_script_id();

        if ( $handle !== $autocomplete_script_id && $handle !== $this->plugin_slug . '-fulltext'){
            return $tag;
        }

        if ($handle == $autocomplete_script_id &&
                strpos($tag, 'acp-magento.js?mode=woocommerce&') === false &&
                (strpos($tag, 'acp-magento.js') !== false) ){
            $autocomplete_params = WCISAutocomplete::get_autocomplete_script_params($this->products_per_page);
            $tag = str_replace('acp-magento.js', 'acp-magento.js?' . $autocomplete_params, $tag);
        }
    ...
    function process_instantsearchplus_request($req){
        if (array_key_exists('instantsearchplus', $req->query_vars)){
            if ($req->query_vars['instantsearchplus'] == 'version'){
                ...
                $server_endpoint = self::ISP_SERVER_ENDPOINT;
                if (!$server_endpoint){
                    $server_endpoint = self::get_isp_server_url();
                }
                ...
                $response = array(
                    ...
                    'wcis_timeframe'        => get_option('wcis_timeframe'),
                    ...
                    'is_serp_config_set'    => !empty(get_option('serp_config'))
                );
                ...
                exit();
            } elseif ($req->query_vars['instantsearchplus'] == 'future_updates'){
                echo json_encode(get_option('cron_update_product_list_by_date'));
                exit();
                ...
                $identifier = $req->query_vars['instantsearchplus_parameter'];
                if (class_exists('WDP_Frontend')) {
                    $temp = new WDP_Frontend();
                    if (method_exists($temp, 'woocommerce_product_get_regular_price')) {
                        add_filter( 'woocommerce_product_get_regular_price', array( $temp, 'woocommerce_product_get_regular_price' ), 100, 2 );
                    }
                    if (method_exists($temp, 'woocommerce_product_get_sale_price')) {
                        add_filter( 'woocommerce_product_get_sale_price', array( $temp, 'woocommerce_product_get_sale_price' ), 100, 2 );
                    }
                }
                ...
                try{
                    ...
                    foreach ($active_plugins as $pl) {
                        if (in_array($pl, $conflicting_plugins)) {
                            ...
                            if ( strpos($pl, 'instantsearch-for-woocommerce') === false  &&
            ...
            elseif (self::get_query_param($req, 0)  == 'set_autocomplete_version' &&
                self::is_query_auth($req)) {
                $version = self::get_query_param($req, 2);
                WCISAutocomplete::set_autocomplete_version($version, true);
            }
            elseif (self::get_query_param($req, 0)  == 'set_serp_version' &&
                self::is_query_auth($req)) {
                $version = self::get_query_param($req, 2);
                WCISServing::set_serp_or_category_version('serp', $version, true);
            }
            elseif (self::get_query_param($req, 0) == 'set_category_version' &&
                self::is_query_auth($req)) {
                $version = self::get_query_param($req, 2);
                WCISServing::set_serp_or_category_version('category', $version, true);
            }
            elseif (self::get_query_param($req, 0) == 'update_serp_config' &&
                self::is_query_auth($req)) {
                WCISServing::update_serving_config(true);
            }
            elseif (self::get_query_param($req, 0) == 'display_serp_config' &&
                self::is_query_auth($req)) {
                WCISServing::display_serving_config();
            }
        }
    }

    private function is_query_auth($req) {
        $auth_key = get_option('authentication_key');
        return ( array_key_exists('instantsearchplus_parameter', $req->query_vars) &&
            $req->query_vars['instantsearchplus_parameter'] == $auth_key);
    }

    public static function get_query_param($req, $i) {
        switch ($i) {
            case 0: return $req->query_vars['instantsearchplus'];
            case 1: return $req->query_vars['instantsearchplus_parameter'];
            case 2: return $req->query_vars['instantsearchplus_second_parameter'];
            case 3: return $req->query_vars['instantsearchplus_third_parameter'];
            default: return '';
        }
    }
    ...
    function wcis_remove_all_filters(){
        ...
        if (is_plugin_active('shortcode-pagination-for-woocommerce/jck-woo-shortcode-pagination.php')
                ...
                || is_plugin_active('us-core/us-core.php')
                || is_plugin_active('facetwp/index.php')) {
            remove_all_filters('pre_get_posts');
        }
        if (class_exists('WDP_Frontend')) {
            $temp = new WDP_Frontend();
...
//        global $wp_query;
        ...
        if(isset($wp_query) && !is_admin() && $wp_query->is_search() && $wp_query->is_main_query()){
...
// 			$q = urldecode((isset($query['s'])) ? $query['s'] : get_search_query());
            ...
            $server_endpoint = self::ISP_SERVER_ENDPOINT;
            if (!$server_endpoint || strlen($server_endpoint) < 10){
                $server_endpoint = self::get_isp_server_url();
            }

            $url = $server_endpoint . 'wc_search';
...
// 				$wp_query->query_vars['s'] = urldecode($q);
            ...
            if (isset($wp_query) && !is_admin() && $wp_query->is_search()){
                return $wp_query;
            } else {
                return self::on_fulltext_disable_query($wp_query);
            }
...
// 		    if there are alternatives(did you mean) or search query not equals to "result for query" (typo)
    ...
    public function posts_search_handler($search){
        global $wp_query;
        if(isset($wp_query) && !is_admin() && is_search() && !$this->fulltext_disabled){
...
// 		}
    ...
    function get_isp_search_box_form( $attr ) {
        ...
        $form = '<form role="search" class="isp_search_box_form" isp_src="shortcode" name="isp_search_box" action="' . $action_url . '" style="width:'.$search_box_width_form.$attr['unit'].'; float:none;">';
     ...
     * @return string
     */
    public function wcis_serp_results_shortcode()
    {
        if (WCISServing::is_svelte_serp()) {
            return WCISServing::get_svelte_serving();
        } else {
            return self::wcis_jquery_serp_results();
        }
    }
     ...
     * @return string
     */
    public function wcis_jquery_serp_results(){
...
//             $session_id = WC()->session->get_customer_id( );        // session_token
            ...
            if ($event == 'success'){
                ...
                foreach($order_array as $product){
                    $cart_product[] = array('product_id' =>      $product['product_id'],
                        ...
                        'currency' =>        "$currency" );
                    self::on_product_update($product['product_id'], 'update', true);
                }
            } else {  // $event == 'cart'
            ...
            if(function_exists('fsockopen') && $event == 'cart') {
                $args = http_build_query($body, '', '&amp;');
                $url .= '?' . $args;
                self::post_without_wait($url, array(), $type = 'GET');
                return;
            }
            else {
                $args = array (
...
SQL;
     ...
     * @param $post_id
     */
    private function check_for_scheduled_sales($post_id, $var_id=null)
    {
        if (!$var_id) {
            $var_id = $post_id;
        }
        $sale_price_dates_from = get_post_meta($var_id, '_sale_price_dates_from', true);
        $sale_price_dates_to = get_post_meta($var_id, '_sale_price_dates_to', true);
    ...
    /**
     * @param $excluded_wp_rocket - the js files that should be excluded from minify-combine or defer by wp-rocket plugin
     * @return array
     */
    public function wp_rocket_exclude_js($excluded_wp_rocket)
    {
        $excluded_isp_arr = array(
            'woo.instantsearchplus.com',
            'acp-magento.appspot.com',
        );

        foreach ($excluded_isp_arr as $excluded_ips) {
            array_push($excluded_wp_rocket, $excluded_ips);
        }

        return $excluded_wp_rocket;
    }
    ...
    public function set_hpos_compatibility() {
        if ( class_exists( \Automattic\WooCommerce\Utilities\FeaturesUtil::class ) ) {
            \Automattic\WooCommerce\Utilities\FeaturesUtil::declare_compatibility(
                    'custom_order_tables', 'instantsearch-for-woocommerce.php', true );
            error_log('hpos comaptibility activated!');
        }
        else {
            error_log('hpos comaptibility was not activated!');
        }
